...
def write_results(prediction, confidence, num_classes, nms_conf = 0.4):
    ...
    for ind in range(batch_size):
        ...
        for cls in img_classes:
            ...
            for i in range(idx):
                try:
                    ious = bbox_iou_detect(image_pred_class[i].unsqueeze(0),image_pred_class[i+1:])
                except ValueError:
                    break
                except IndexError:
                    break
...
def object_detect(img_path_list,inp_dim,class_names, bboxes_predict, ratio_list,colors,output_path = './output/'):
    for i in range(len(img_path_list)):
        ...
        bbox = change_bboxes(predict_bbox, ratio, img_w,img_h,inp_dim)
        draw_bbox(img, bbox,colors,class_names)
        
        output_path =os.path.join(output_path,'det-%s'%(os.path.basename(img_path_list[i])) )
        cv2.imwrite(output_path,img)

# before draw bboxes transform the bboxe to make the bbox suit for the original image
def change_bboxes(bbox_predict, ratio, img_w,img_h, inp_dim):
    """
...
def draw_bbox(image, bboxes,colors,classes):
    """
